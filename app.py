...
def handle_GET_game(pool, current_params):
    ...
    save_new_game(pool, new_game_id, topic_id, points_to_win, round_time, rounds_finished, teams_ids)
...
def handle_POST_game_configure(pool, body):
    ...
    save_new_game(pool, game_id, topic_id, points_to_win, round_time, rounds_finished, teams_ids)
...
def handle_PATCH_answer(pool, current_params, body):
    ...
    if total_stats == int(game['points_to_win']):
        ...
        advice_request = requests.get('https://api.adviceslip.com/advice')
        advice = advice_request.json()['slip']['advice']
...
def save_new_game(pool, game_id, topic_id, points_to_win, round_time, rounds_finished, teams_ids):
    text = f"""
        UPSERT INTO games ( game_id, topic_id, points_to_win, round_time, rounds_finished, team_ids )
        VALUES ( {game_id}, {topic_id}, {points_to_win}, {round_time}, {rounds_finished}, "{teams_ids}");
    """
    return ydb_request(pool, text)
...
def connect_DB(iam_token):
    ...
    pool = ydb.SessionPool(driver, workers_threads_count=10)
    ...
    for i in range(retries):
        try:
            return pool.retry_operation_sync(lambda s: s.transaction().execute(
                text,
                commit_tx=True,
                settings=ydb.BaseRequestSettings().with_timeout(5).with_operation_timeout(5)
            ))
